...
function add_expense() {
  ...
  t.push(Number(amt));
    ...
    expense_per_category[category] += parseInt(amt);
    update_sum();
...
function add_category() {
  ...
  if (new_category_value) {
    ...
    new_category_span.innerHTML = `
      <span>${new_category_value}<span class="sum"> - ${expense_per_category[count]} Rs spent</span></span>`;
...
function add_expense_page() {
  let addNewExpense = document.getElementsByClassName("add-new-expense")[0];
  if (addNewExpense.style.display === "block") {
...
function add_category_page() {
 ...
 function ExportToExcel(type, fn, dl) {
            ...
            return dl ?
                ...
                XLSX.writeFile(wb, fn || ('MyExpenses.' + (type || 'xlsx')));
        }
